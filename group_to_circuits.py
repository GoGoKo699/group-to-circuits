...
def check_faithful(params):
    ...
    for i in range(s):
        ...
        #(c)
        c3=sub_circuit_0(ru)
        c3_params=list(a)+list(b)
        c3.set_parameters(c3_params)
        state3=c3().state()
        loss_3=loss_3-np.abs(state3[0])
        #(d)
        c4=sub_circuit_0(ru)
        c4_params=list(c)+list(c)
        c4.set_parameters(c4_params)
        state4=c4().state()
        loss_4=loss_4-np.abs(state4[0])
    return -loss_1/s,-loss_2/s,-loss_3/s,-loss_4/s
